#!/usr/bin/env python3
"""
Flipped Voltage Follower (FVF) Circuit Generator
"""
import os
import sys
from pathlib import Path

# Setup path to find glayout module
_here = Path(__file__).resolve()
_glayout_repo_path = _here.parent.parent.parent.parent.parent.parent
pwd_path = Path.cwd().resolve()

# Fallback hard-coded path if relative logic fails
...
def fvf_netlist(fet_1: Component, fet_2: Component) -> str:
    """Generate SPICE netlist string for flipped voltage follower - gymnasium compatible"""
    
    # Get the SPICE netlists directly from components
    fet_1_spice = fet_1.info.get('netlist', '')
    fet_2_spice = fet_2.info.get('netlist', '')
    
    if not fet_1_spice or not fet_2_spice:
        raise ValueError("Component netlists not found")
    
    # Create the FVF SPICE netlist by combining the primitives
    # Port names must match the labels added by sky130_add_fvf_labels:
    # VIN (input gate), VBULK (bulk/substrate), VOUT (output), Ib (bias current)
    fvf_spice = f"""{fet_1_spice}

{fet_2_spice}

.subckt FLIPPED_VOLTAGE_FOLLOWER VIN VBULK VOUT Ib
* Input FET: gate=VIN, drain=Ib, source=VOUT
X0 Ib VIN VOUT VBULK NMOS
* Feedback FET: gate=Ib, drain=VOUT, source=VBULK
X1 VOUT Ib VBULK VBULK NMOS
...
def  flipped_voltage_follower(
    ...
    **kwargs: any kwarg that is supported by nmos and pmos
    ...
    # Store netlist as string for gymnasium compatibility - LVS method supports this directly
    netlist_string = fvf_netlist(fet_1, fet_2)
