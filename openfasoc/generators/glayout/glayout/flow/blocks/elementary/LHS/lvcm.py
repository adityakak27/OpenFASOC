...
from glayout.flow.blocks.elementary.LHS.fvf import fvf_netlist, flipped_voltage_follower, get_component_netlist
...
def add_lvcm_labels(lvcm_in: Component,
    ...
    # Preserve netlist info before flattening
    netlist_info = lvcm_in.info.get('netlist', None)
    flattened = lvcm_in.flatten()
    
    # Restore netlist info after flattening
    if netlist_info is not None:
        flattened.info['netlist'] = netlist_info
    
    return flattened 

def create_fvf_netlist(fvf_component: Component, name_suffix: str) -> Netlist:
    """Create a Netlist object for FVF component from its internal structure"""
    # FVF has two FETs internally, we need to get them
    # The FVF circuit has nodes: VIN, VBULK, VOUT, Ib
    fvf_netlist = Netlist(circuit_name=f'FVF_{name_suffix}', nodes=['VIN', 'VBULK', 'VOUT', 'Ib'])
    
    # Get the netlist string and convert it to Netlist object representation
    # For now, we'll represent this as a subcircuit with the FVF topology
    # Since FVF stores SPICE strings, we'll store it as source_netlist
    if 'netlist' in fvf_component.info:
        fvf_netlist.source_netlist = fvf_component.info['netlist']
    
    return fvf_netlist

def create_fet_netlist(fet_component: Component, name_suffix: str) -> Netlist:
    """Create a Netlist object for FET component from its internal structure"""
    # FET has nodes: D (drain), G (gate), S (source), B (bulk/body)
    fet_netlist = Netlist(circuit_name=f'FET_{name_suffix}', nodes=['D', 'G', 'S', 'B'])
    
    # Get the netlist string and convert it to Netlist object representation
    # Since FET stores SPICE strings, we'll store it as source_netlist
    if 'netlist' in fet_component.info:
        fet_netlist.source_netlist = fet_component.info['netlist']
...
def low_voltage_cmirr_netlist(bias_fvf: Component, cascode_fvf: Component, fet_1_ref: ComponentReference, fet_2_ref: ComponentReference, fet_3_ref: ComponentReference, fet_4_ref: ComponentReference) -> Netlist:
        ...
        # Create Netlist objects for the FVF components
        bias_fvf_netlist = create_fvf_netlist(bias_fvf, 'bias')
        cascode_fvf_netlist = create_fvf_netlist(cascode_fvf, 'cascode')
        
        # Create Netlist objects for the FET components
        fet_1_netlist = create_fet_netlist(fet_1_ref.parent, 'fet1')
        fet_2_netlist = create_fet_netlist(fet_2_ref.parent, 'fet2')
        fet_3_netlist = create_fet_netlist(fet_3_ref.parent, 'fet3')
        fet_4_netlist = create_fet_netlist(fet_4_ref.parent, 'fet4')
        
        netlist.connect_netlist(bias_fvf_netlist, [('VIN','IBIAS1'),('VBULK','GND'),('Ib','IBIAS1'),('VOUT','local_net_1')])
        netlist.connect_netlist(cascode_fvf_netlist, [('VIN','IBIAS1'),('VBULK','GND'),('Ib', 'IBIAS2'),('VOUT','local_net_2')])
        fet_1A_ref=netlist.connect_netlist(fet_2_netlist, [('D', 'IOUT1'),('G','IBIAS1'),('B','GND')])
        fet_2A_ref=netlist.connect_netlist(fet_4_netlist, [('D', 'IOUT2'),('G','IBIAS1'),('B','GND')])
        fet_1B_ref=netlist.connect_netlist(fet_1_netlist, [('G','IBIAS2'),('S', 'GND'),('B','GND')])
        fet_2B_ref=netlist.connect_netlist(fet_3_netlist, [('G','IBIAS2'),('S', 'GND'),('B','GND')])
...
if __name__=="__main__":
    ...
    base_component = low_voltage_cmirror(sky130_mapped_pdk)
    # Store the netlist before adding labels
    netlist_obj = base_component.info.get('netlist')
    low_voltage_current_mirror = add_lvcm_labels(base_component, sky130_mapped_pdk)
    # Explicitly restore netlist after adding labels
    if netlist_obj is not None:
        low_voltage_current_mirror.info['netlist'] = netlist_obj
