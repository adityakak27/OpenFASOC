...
class CodeFixDataset(Dataset):
    """Torch dataset that turns JSON samples into LM training pairs."""
        ...
        with path.open() as f:
            if path.suffix == ".jsonl":
                ...
                for line in f:
                    ...
                    # Handle markdown-formatted JSON blocks
                    if line.startswith("```json"):
                        in_json_block = True
                        current_json = ""
                        continue
                    elif line.startswith("```") and in_json_block:
                        # End of JSON block, parse the accumulated JSON
                        ...
                        # Direct JSON line (fallback for non-markdown format)
                        try:
                            data.append(json.loads(line))
                        except json.JSONDecodeError as e:
                            LOGGER.warning(f"Failed to parse JSON line: {e}")
                
                # Handle any remaining JSON block that wasn't closed
...
def freeze_lower_layers(model, train_last_n: int = 0):
    """Freeze all transformer blocks except the last `train_last_n`."""
    if train_last_n <= 0:
        LOGGER.info(" Training all layers (train_last_n_layers <= 0)")
        return

    # Try to find transformer layers in common model architectures
    layers = None
    if hasattr(model, 'model') and hasattr(model.model, 'layers'):
        layers = model.model.layers  # Llama/CodeLlama style
    elif hasattr(model, 'transformer') and hasattr(model.transformer, 'h'):
        layers = model.transformer.h  # GPT style
    elif hasattr(model, 'encoder') and hasattr(model.encoder, 'block'):
        layers = model.encoder.block  # T5 style
...
def freeze_layers_old_approach(model, n_layers: int, train_embeddings: bool = False):
    """Original freeze first N layers approach (kept for compatibility)."""
    if n_layers <= 0:
        return
...
def main():
    ...
    for model_path in args.model_paths:
        ...
        # Build datasets with (potentially) model-specific tokenizer
        ...
        checkpoint_dirs = list(out_dir.glob("checkpoint-*"))
        for checkpoint_dir in checkpoint_dirs:
            if checkpoint_dir.is_dir():
                import shutil
                shutil.rmtree(checkpoint_dir)
                LOGGER.info(f"   Deleted checkpoint: {checkpoint_dir.name}")
        ...
        if args.use_lora:
            ...
            LOGGER.info(f"✅ LoRA adapter saved to {out_dir} (~40MB)")
            LOGGER.info(f"💡 To use: Load base model + this adapter (see load_lora_model.py)")
            LOGGER.info(f"💡 To merge for deployment: Use merge_lora_to_full_model.py")
        else:
            # Full weight training fallback
            trainer.save_model()
            tokenizer.save_pretrained(str(out_dir))
            LOGGER.info(f"✅ Full model saved to {out_dir}")
